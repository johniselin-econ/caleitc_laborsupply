...
foreach out of local outcomes {
	...
	** Save table locally
	esttab 	est_`out'_1 est_`out'_2 est_`out'_3 est_`out'_4 using			///
		"${results}tables/tab_het_adults_`ct'.tex",									///
    ...
    ** For first outcome, create spec indicators table
    if `ct' == 1 {
		...
		** Save
		esttab 	est_`out'_1 est_`out'_2 est_`out'_3 est_`out'_4 using				///
			"${results}tables/tab_het_adults_end.tex",										///
			booktabs fragment nobaselevels replace nomtitles nonumbers nolines	///
			stats(`stats_list', 												///
				fmt("`stats_fmt'") 												///
				labels(`stats_labels'))											///
...
graph export "${results}tables/fig_tab_het_adults.jpg", as(jpg) quality(100) replace
