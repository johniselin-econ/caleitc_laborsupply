/*******************************************************************************
File Name:      04_appA_tab_ny_placebo.do
Creator:        John Iselin
Date Update:    January 2026

Purpose:        Creates Appendix Table: NY Placebo Test
                Triple-difference estimates using New York as placebo treatment
                state. California is excluded from the sample.
...
** Start log file
capture log close log_04_appA_tab_ny_placebo
log using "${logs}04_appA_tab_ny_placebo_log_${date}", ///
    name(log_04_appA_tab_ny_placebo) replace text
...
** Create main DID variables using NY as placebo treatment
gen ny = (state_fips == 36)
gen post = (year > 2014)
gen treated = (qc_present == 1 & ny == 1 & post == 1)
...
foreach out of local outcomes {
    ...
    ** SPEC 1: Basic triple-diff FEs only
    ...
    add_table_stats, outcome(`out') treatvar(treated) ///
        postvar(post) statevar(ny) qcvar(qc_present) weightvar(weight)
    ...
    ** Save table locally
    esttab est_`out'_1 est_`out'_2 est_`out'_3 est_`out'_4 using ///
        "${results}tables/tab_ny_placebo_`ct'.tex", ///
    ...
    ** For first outcome, create spec indicators table
    if `ct' == 1 {
        ...
        esttab est_`out'_1 est_`out'_2 est_`out'_3 est_`out'_4 using ///
            "${results}tables/tab_ny_placebo_end.tex", ///
            booktabs fragment nobaselevels replace nomtitles nonumbers nolines ///
            stats(`stats_list', ///
                fmt("`stats_fmt'") ///
                labels(`stats_labels')) ///
...
clear
log close log_04_appA_tab_ny_placebo
