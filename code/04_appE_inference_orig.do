...
program define ferman_pinto_boot_ind, rclass
    ...
    ** Generate W (components will sum together)
    gen W = .
    qui replace W = Pr_q1t * eta_qjt if post == 1 & `G' == 1
    qui replace W = -Pr_q1t * eta_qjt if post == 1 & `G' == 0
    qui replace W = -Pr_q1t * eta_qjt if post == 0 & `G' == 1
    qui replace W = Pr_q1t * eta_qjt if post == 0 & `G' == 0
    ...
    ** Collapse to state level
    keep `CL' `T' `G' ca W q P_qjt
    duplicates drop `CL' `T' `G' ca W q P_qjt, force
    collapse (mean) ca (sum) W q P_qjt, by(`CL')
    ...
    ** Run residual bootstrap with and without correction
    forvalues b = 1/`B' {
        ...
        ** Fill in W from bootstrap assigned state (uncorrected)
        gen W_tilde = W[h2]

        ** Fill in W from bootstrap assigned state (corrected)
...
program define ri_bs, rclass
    ...
    ** STEP 4: Bootstrap for each RI permutation
    ...
    drop control_states _reghdfe_resid temp_*
