...
** Define outcome variable (household earned income)
...
** Load ACS data
use weight `outcome' incearn_real `controls' `unemp' `minwage' qc_* year ///
...
forvalues m = 1/3 {
	...
	** Save table locally
	esttab 	est_`m'_1 est_`m'_2 est_`m'_3 est_`m'_4 using					///
		"${results}tables/tab_hh_earn_`m'.tex",										///
    ...
    ** For first model, create spec indicators table
    if `m' == 1 {
		...
		** Save
		esttab	est_`m'_1 est_`m'_2 est_`m'_3 est_`m'_4 using				///
			"${results}tables/tab_hh_earn_end.tex",									///
			booktabs fragment nobaselevels replace nomtitles nonumbers nolines	///
			stats(`stats_list', 											///
				fmt("`stats_fmt'") 											///
				labels(`stats_labels'))										///
